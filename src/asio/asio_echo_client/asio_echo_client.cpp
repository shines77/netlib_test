    ...
    const std::string & port, uint32_t packet_size)
{
    ...
    try
    {
        ...
        //auto endpoint_iterator = resolver.resolve( {"192.168.2.154", "8090"} );
        auto endpoint_iterator = resolver.resolve( { ip, port } );
        test_qps_client client(io_service, endpoint_iterator, g_mode, 32768, packet_size);
...
void print_usage(const std::string & app_name)
{
    std::cerr << std::endl
              ...
              << "           mode:        Client run mode, you can choose pingpong, qps, latency and throughput." << std::endl
              << std::endl
...
int main(int argc, char * argv[])
{
    ...
    int has_mode;
    if (argc > 1 && std::strncmp(argv[1], "mode=", sizeof("mode=") - 1) == 0)
        has_mode = 1;
    else
        has_mode = 0;
    ...
    if (argc > (3 + has_mode))
        packet_size = std::atoi(argv[3 + has_mode]);
    if (packet_size <= 0)
        packet_size = 64;
    ...
    else if (g_mode == mode_throughput)
        run_throughput_client(app_name, ip, port, packet_size);
